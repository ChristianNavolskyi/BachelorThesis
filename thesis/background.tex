...
which derived from the Red-Black Tree and the B+ Tree and it therefore supports fast insertions as well as fast lookups.\cite{Abubakar2014}
...
Each "Client thread" calls the "Workload executor" to perform a operation on the database.
...
but failed at all workloads using the dataset created by their date generator.\cite{Capota2015}
