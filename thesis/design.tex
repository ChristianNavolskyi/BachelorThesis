...
\todo{create graphic and describe its content. Graphic should have numbers on its edges, maybe use puml}
...
\begin{itemize}
  ...
  \item Class to store the order of operations
  \item Class to get random graph component identifiers
...
\textbf{DB::update} \newline
For this operation we also need an existing graph component picked by the \linebreak
$ RandomGraphComponentGenerator $ to get a valid key identifier and the kind of graph component we are going to update.
...
A common implementation is shown in listing~\ref{lst:databaseTemplate}.
...
\subsection{Summary}
\todo{Parameters!!, where used general workflow.}
